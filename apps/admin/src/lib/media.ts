...
export function checkFile(file: File, types: string[]): string | null {
  if (!types.includes(file.type)) return types === IMAGE_TYPES ? 'Format non accepté : JPG, PNG ou WebP.' : 'Format non accepté : PDF, Word, Excel ou OpenDocument.';
...
export async function uploadFile(file: File, folder = 'documents'): Promise<UploadedFile> {
  ...
  const response = await api<{ data: { file: UploadedFile } }>('/api/media-items/upload', { method: 'POST', body: form });
...
export function describeFile(file: Pick<UploadedFile, 'ext' | 'size'>): string {
  ...
  const size = file.size < 1024 ? `${Math.max(1, Math.round(file.size))} Ko` : `${(file.size / 1024).toFixed(1).replace('.', ',')} Mo`;
  return `${format} · ${size}`;
}
