...
- `COMMUNEO_LEGAL_NAME`, `COMMUNEO_LEGAL_ADDRESS`, `COMMUNEO_SIRET`, `COMMUNEO_BILLING_EMAIL`, `COMMUNEO_VAT_RATE` — issuer of the quotes (#312); unset fields print « [à compléter] » on the PDF
...
- `src/api/quote/` — online quote (#312): offer from the INSEE population read server-side (`communePopulation`), price grid in `@communeo/core` (`pricing.ts`, example prices, analysis in `docs/business-plan-couts.md`); an admin validates the quote (SIRET checked, signatory and role, time and IP kept, PDF rendered with pdfkit and archived in `quote.pdf` with its SHA-256); a signed quote is the live request the team approves in « À valider » (quote `accepted`, or `rejected` with a reason). Issuer identity and VAT: `COMMUNEO_LEGAL_NAME`, `COMMUNEO_LEGAL_ADDRESS`, `COMMUNEO_SIRET`, `COMMUNEO_BILLING_EMAIL`, `COMMUNEO_VAT_RATE` (0 = franchise, art. 293 B du CGI)
