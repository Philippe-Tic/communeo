...
- Le domaine de la preview des brouillons (`PREVIEW_DOMAIN=preview.tondomaine.fr`, certificat : `certbot certonly --webroot -w /var/www/certbot -d preview.tondomaine.fr`) et `PREVIEW_API_TOKEN` (comme `STRAPI_API_TOKEN`, affiché dans les logs au premier démarrage de Strapi)
