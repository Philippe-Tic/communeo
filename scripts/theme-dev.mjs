...
const { positionals, values } = parseArgs({
  ...
  options: { variant: { type: 'string', default: 'complete' }, logo: { type: 'string', default: 'horizontal' }, port: { type: 'string', default: '4321' }, alert: { type: 'boolean', default: false } },
});
...
const child = spawn('pnpm', ['--filter', '@communeo/renderer', 'exec', 'astro', 'dev', '--port', values.port], {
  stdio: 'inherit',
  env: { ...process.env, THEME: theme, DATA_SOURCE: 'fixtures', FIXTURE_VARIANT: values.variant, FIXTURE_LOGO: values.logo, FIXTURE_ALERT: values.alert ? 'critical' : '' },
});
