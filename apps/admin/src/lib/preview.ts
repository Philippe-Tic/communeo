...
export type PreviewTarget = { type: 'page' | 'article' | 'evenement' | 'official-document'; documentId: string };
...
export const previewQuery = (target: PreviewTarget | null, options: { theme?: string; slug?: string } = {}) =>
  queryOptions({
    queryKey: ['preview', target?.type, target?.documentId, options.theme, options.slug],
    queryFn: () => api<{ url: string; expiresAt: string }>('/api/preview/token', { method: 'POST', json: { ...target, ...(options.theme ? { theme: options.theme } : {}) } }),
    enabled: !!target,
